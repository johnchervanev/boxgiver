class LandingPageController < ActionController::Metal
  ...
  def index
    ...
    @categories = @current_community.categories.where("parent_id IS NULL").order(id: :asc).limit(8)
    @popular_categories = @current_community.categories.where("parent_id IS NULL").order('created_at asc').limit(8)
